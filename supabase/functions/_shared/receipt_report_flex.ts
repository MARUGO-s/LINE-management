...
import { fetchManualMonthSales, type ManualMonthSalesRecord } from "./manual_month_sales.ts"
import {
...
export type ReceiptReportYoyComparison = {
  ...
  /** 前年同期間の営業日数（レシートがある distinct receipt_date） */
  priorOperatingDayCount: number | null
}

/** レシート解析 Flex 用の前年比行（line-webhook の baseline 形式） */
export type ReceiptYoyKvRow = {
  label: string
  value: string
  valueColor?: string
  margin?: "md"
...
function countInclusiveCalendarDays(periodStartDate: string, periodEndDate: string): number {
...
function calendarDaysInMonth(yyyyMm: string): number {
  const m = yyyyMm.match(/^(\d{4})-(\d{2})$/)
  if (!m) return 30
  return new Date(Date.UTC(Number(m[1]), Number(m[2]), 0)).getUTCDate()
}

/** 途中期間の前年比: 手入力に営業日数があれば「今年営業日÷昨年営業日」、なければ暦日数比 */
export function resolveManualMonthYoyScale(
  periodStartDate: string,
  periodEndDate: string,
  manual: ManualMonthSalesRecord,
  currentPeriodOperatingDayCount: number | null,
): number {
  ...
  if (
    manualOp != null &&
    manualOp > 0 &&
    currentPeriodOperatingDayCount != null &&
    currentPeriodOperatingDayCount > 0
  ) {
    return currentPeriodOperatingDayCount / manualOp
  }

  const priorMonth = periodStartDate.slice(0, 7)
  const monthDays = calendarDaysInMonth(priorMonth)
...
export async function loadReceiptReportYoyComparison(
  ...
  const currentAggregate = await loadReceiptReportAggregateForStoreByReceiptDate(
    supabase,
    key,
    periodStartDate,
    periodEndDate,
  ...
  if (manual) {
    ...
    if (isFullCalendarMonthPeriod(periodStartDate, periodEndDate)) {
      ...
      const scale = resolveManualMonthYoyScale(
        periodStartDate,
        periodEndDate,
        manual,
      ...
      if (scale > 0) {
        ...
        priorPartyCount = manual.party_count != null
          ? Math.round(manual.party_count * scale)
          : null
        priorGuestCount = manual.guest_count != null
          ? Math.round(manual.guest_count * scale)
          : null
      }
    }
  }
  ...
  return {
    ...
    priorPartyCount,
    priorGuestCount,
    priorOperatingDayCount: priorAggregate?.operatingDayCount ?? null,
  }
}
...
function formatYoyMetricValueLine(
  current: number,
  prior: number | null,
...
function pushReceiptYoyKvMetricRow(
  rows: ReceiptYoyKvRow[],
  label: string,
  current: number,
  prior: number | null,
  formatAbsDiff?: (diff: number) => string,
...
function formatYoyPriorPeriodLabel(start: string, end: string): string {
...
function formatYoyPctCompact(current: number, prior: number | null): { text: string; color: string } {
  if (prior == null || prior <= 0) return { text: "—", color: "#888888" }
  const change = formatYoyPercentChange(current, prior)
  if (!change) return { text: "—", color: "#888888" }
  return { text: change.text.trim(), color: change.color }
}

/** レシート解析 Flex 用：【前年同月比】を2行（baseline 形式・span 不使用） */
export function buildReceiptYoyCompactKvRows(
  ...
  const metrics: Array<{ name: string; current: number; prior: number | null }> = [
    { name: "売上", current: aggregate.totalGrossSalesYen, prior: yoy.priorGrossSalesYen },
    { name: "組", current: aggregate.totalPartyCount, prior: yoy.priorPartyCount },
    { name: "客", current: aggregate.totalGuestCount, prior: yoy.priorGuestCount },
    { name: "営業日", current: aggregate.operatingDayCount, prior: yoy.priorOperatingDayCount },
  ]
  ...
  return [
    { label: "【前年同月比】", value: periodLabel, margin: "md" },
    { label: "比較", value: metricParts.join("  ") },
  ]
}

/** レシート解析返信など、baseline KV 向けの前年比行（前年データなしは「—」で常に4行） */
export function buildReceiptYoyKvRows(
  aggregate: ReceiptReportAggregate,
  yoy: ReceiptReportYoyComparison,
): ReceiptYoyKvRow[] {
  const rows: ReceiptYoyKvRow[] = []
  pushReceiptYoyKvMetricRow(rows, "売上", aggregate.totalGrossSalesYen, yoy.priorGrossSalesYen, formatSignedYenDiffText)
  pushReceiptYoyKvMetricRow(
    rows,
    "組数",
  ...
  pushReceiptYoyKvMetricRow(
    rows,
    "営業日数",
    aggregate.operatingDayCount,
    yoy.priorOperatingDayCount,
    (d) => formatSignedCountDiffText(d, "日"),
  )
  return rows
}
...
function receiptCountMatchesOperatingDays(aggregate: ReceiptReportAggregate): boolean {
...
export async function buildReceiptReportFlexMessages(
  ...
  if (countMismatch) {
    bodyContents.push({
      type: "text",
      text:
        `⚠ レシート${aggregate.receiptCount.toLocaleString("ja-JP")}件と営業日${aggregate.operatingDayCount.toLocaleString("ja-JP")}日が一致しません（通常は同数です）`,
      size: "sm",
      weight: "bold",
      color: "#c62828",
      margin: "md",
  ...
  bodyContents.push(
    ...
    flexBaselineRow(
      "営業日数",
      `${aggregate.operatingDayCount.toLocaleString("ja-JP")} 日`,
      countMismatch ? "#c62828" : "#1F1F1F",
    ),
    flexBaselineRow(
      "レシート件数",
      `${aggregate.receiptCount.toLocaleString("ja-JP")} 件`,
      countMismatch ? "#c62828" : "#1F1F1F",
    ),
  )

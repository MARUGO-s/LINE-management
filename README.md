...
- トークから **保存メディアの署名付きダウンロード URL** を返す文言: `メディアURL`（省略時は最新 1 件）、`メディアURL 2`〜`3`（新しい順に最大 3 件）。ルームのメディア保存 ON かつユーザーの `can_media_access` が前提（`line-webhook`）。**友だち 1:1 トークではメディアを保存しない**ため、このコマンドは案内のみ（保存があるグループ／複数人トークで利用）。

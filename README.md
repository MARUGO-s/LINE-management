...
- トークから **保存メディアの署名付きダウンロード URL** を返す文言: `メディアURL`（省略時は最新 1 件）、`メディアURL 2`〜`3`（新しい順に最大 3 件）。**友だち 1:1 でも**、メディア保存が有効でユーザーの `can_media_access` がある場合はグループと同様に保存・URL 返却（`line-webhook`）。1:1 の**テキスト**は従来どおり履歴 DB には載せず、**画像・ファイル等のメディア行のみ**保存する。

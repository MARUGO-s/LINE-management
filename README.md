...
| `can_calendar_update` | カレンダー更新/一覧の可否 |
...
- 第一段階でヒット 0 件の場合のみ、確認応答を経て資料ライブラリ検索へ進む（`はい` / `いいえ`、30 分有効）。
...
| `line_user_permissions` | LINE user 単位の機能権限（検索/資料検索/予定作成/予定更新/メディア/有効） |
...
- `custom.calendar_pending_edge_function_url` … `calendar-pending-cron` の URL
- `custom.cron_auth_token` … Edge 呼び出し用 Bearer（**マイグレーション内の anon JWT デフォルトに依存しない**）

`invoke_calendar_pending_cron` は最新マイグレーションで強化され、`custom.calendar_pending_edge_function_url` または `custom.cron_auth_token` が未設定の場合は **警告ログを出して安全にスキップ**します。  
本番では4つの `custom.*` 設定を必ず投入してください。
...
- **Gitleaks** による Secret Scan（`.gitleaks.toml`）。履歴上の旧マイグレーションに含まれる資格情報デフォルトに依存せず、`custom.*` 設定を明示投入して運用してください。

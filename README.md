...
- **グループ／複数人トーク**ではイベントごとに `line_messages` へ保存。**友だち 1:1 では会話テキストは保存しない**（メディア保存のための行のみ挿入する場合あり）。テキスト以外は `toStoredMessageContent` により種別ごとの説明文（例: `【画像が送信されました】[[MEDIA:…]]`、位置情報・スタンプ等も区別）。
- **友だち 1:1**でも **会話検索・資料ライブラリ検索コマンドは利用可**（ユーザー権限・ルーム設定に従う）。ただし **1:1 の会話テキストは `line_messages` に保存しない**ため、「このルームのみ」の検索ではテキストヒットは基本的に出ない。**全ルーム**指定でグループ等の保存履歴を検索できる。
- 保存対象メディア種別: `image`, `video`, `audio`, `file`（定数 `STORABLE_LINE_MEDIA_TYPES`）。取得後 `line-media` バケットへアップロードし `line_message_media` にメタデータ保存。
- **画像（`image`）**で、MIME が **`image/jpeg` / `image/jpg` / `image/png`** のとき、`GROQ_API_KEY` が設定されていれば **Groq の `meta-llama/llama-4-scout-17b-16e-instruct`** で内容を短文解析し、結果を **`line_message_media.content_preview`** に保存する。管理画面の `media.html` では **「画像解析結果（Llama 4 Scout）」** として表示され、**何が写っているか**を一覧から把握できる（解析失敗・キー未設定・サイズ超過などでは、これまでと同様に `content_preview` が空のままなことがある。詳細は `docs/MEDIA_OCR_AND_CLASSIFICATION.md`）。
- トークから **保存メディアの署名付きダウンロード URL** を返す文言: **`メディアURL` だけでよい**。常に **`line_message_media` 全体から新しい順**・最大 3 件（**`メディアURL 2`〜`3`** で件数指定）。トークルームの区別なし。会話検索の「検索対象外ルーム」は **適用しない**。**`全ルーム` を付けても同じ**（互換のため解釈のみ）。**友だち 1:1 でも**、メディア保存が有効で `can_media_access` があれば保存・URL 返却可。1:1 の**テキスト**は履歴 DB に載せず、**メディア行のみ**保存する。
- メディア: ルーム別・合計容量上限、1 ファイル上限（設定値と LINE 20MB 上限の両方）を実装。
- **保存メディア検索**（`メディア検索` / `保存メディア検索`）: 期間・カテゴリ・送信者・キーワードで `line_message_media` を絞り込み、候補に番号を付けて返す。**キーワード照合**はファイル名と **`content_preview`（ファイルなら先頭付近の抽出テキスト、画像なら Groq 短文）** に対して行う。日本語の自然文は**助詞区切り・文末の除去**などでトークン化し、各部分が**部分一致**すればヒットしやすい（`extractMessageSearchKeywordTokens` / `keywordMatchesHaystacks`）。**LINE の一覧・詳細・`メディアURL` 応答に載せる「解析:」行**は、**ファイル**については抽出テキスト全文ではなく、管理画面 `media.html` の **「解析結果（推定）」** と同じルールの**短文ラベル**のみ（実装: `supabase/functions/_shared/line_media_file_purpose_infer.ts`）。**画像**は従来どおり Groq の短文を短く表示。
...
- リポジトリの **`index.html`** は同一画面の静的版（GitHub Pages 等）。**ユーザー権限**の「除外 N 件」は、**現在表示中のルーム一覧に存在する `room_id` のみ**を数え、ルーム削除後に残った DB 上の古い除外 ID は**件数に含めない**（再読み込みで自動反映。保存時は有効 ID のみ送信）。
...
- Gmail API で予約系メールを検索（デフォルトクエリ: `is:inbox is:unread newer_than:7d (予約 OR reservation OR booking)`）。`GMAIL_ALERT_QUERY` に `is:unread` / `is:inbox` を含めた場合でも、未検知を減らすため必要時はそれらを外した緩和クエリで再検索。
- 通知対象は **一休.comレストラン / 食べログ** の予約通知メールのみ（予約以外の通知・一般メールは除外）。
- 抽出結果は LINE へ **統一テンプレート**で送信。`要望` は提携定型文（予約台帳転記依頼など）を除外し、該当時は `なし`。食べログは `予約番号` を非表示。
- 食べログは `名前 + 電話番号` ベースのDBロジック集計で `予約回数 N回` を表示（AI推定ではない）。同一メールの重複加算は `gmail_message_id` 単位で防止。
...
- キーワードは **空白・読点でトークン分割**のほか、**日本語の連文**では **助詞（の・は・を 等）で区切って**複数トークンにし、**疑問文末の丁寧表現を削る**（例: `haccpの予定表は保存されていますか` → `haccp` / `予定表` など）。**各トークンがすべて**、対象文字列に **部分一致（substring）** すればヒット（AND）。トークンは `expandKeywordVariants` により **類義語展開**（例: `ミーティング` ↔ `meeting` ↔ `会議` 等のグループ、`KEYWORD_SYNONYM_GROUPS`）。
...
| 通知対象メール | 一休.comレストラン / 食べログ の予約通知のみ |
| 食べログ表示 | `予約番号` 非表示、`予約回数 N回` を表示 |
| 食べログ履歴集計 | `tabelog_reservation_visit_events`（重複防止） + `tabelog_reservation_visit_summaries`（`名前 + 電話番号`累積） |
...
- **Gmail予約通知（予約判定厳格化）**: 通知対象を **一休.comレストラン / 食べログ** の予約通知メールに限定。予約以外通知（セキュリティ・レビュー等）は除外。
- **Gmail予約通知（食べログ表示）**: 提携定型文は `要望` から除外して `なし` 表示。`予約番号` は非表示、`予約回数 N回` を表示。
- **食べログ来店履歴（DBロジック）**: `名前 + 電話番号` で来店回数を加算する集計を追加。`tabelog_reservation_visit_events` で同一 `gmail_message_id` の重複加算を防止し、`tabelog_reservation_visit_summaries` で累積管理。RPC: `record_tabelog_reservation_visit`。
...
- **保存メディア検索（LINE）**: キーワードの日本語トークン化（助詞区切り等）と、ファイルの「解析:」行を推定短文のみに変更（`line_media_file_purpose_infer.ts`）。
- **管理 UI（ユーザー権限）**: 会話検索「除外 N 件」バッジを、**削除済みルーム ID を件数に含めない**よう `room_overview` と突き合わせ（`index.html` / `admin-ui`）。
- **管理 UI（ユーザー権限）**: 新規ルームが追加された場合、既存ユーザーの会話検索対象は**初期で未チェック（除外）**になるよう、`room_summary_settings` の INSERT 連動で `excluded_message_search_room_ids` へ自動追加。
- **UIテーマ**: `index.html` / `media.html` / `reservation.html` にライト/ダーク切替ボタンを追加。テーマは `line_reservation_calendar_theme` で共有保持。

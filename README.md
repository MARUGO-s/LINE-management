...
| 種別 | ユーザー側の例 | 検索が届く期間の目安（保持が十分長い場合） | 実装メモ |
|------|----------------|------------------------------------------|----------|
| **通常（第1段→第2段→第3段）** | `会話検索 キーワード` 等（日数指定も最終的に **180 日まで**に制限） | **第1段**: 過去 180 日（保持が短い場合はその範囲）を **一括**検索。**第2段**: ヒット 0 件かつ保持がそれより長い場合、**同一応答内で**保持期間いっぱいまで会話を追加検索（フル相当）。**第3段**: 会話でも 0 件なら **資料ライブラリを検索するか** `はい` / `いいえ` で確認。 | 第1段は `buildMessageSearchPhase1WizardWindows`。第2段は `fullRetentionSearch` 相当の日数で `buildMessageSearchStageDayWindows`（第1段と範囲が同じなら第2段はスキップし第3段へ）。 |
| **フル（ショートカット）** | `会話検索フル キーワード`、`会話検索全履歴 キーワード`、`会話検索裏 キーワード`、`フル会話検索 キーワード` 等 | **第1・2段を省略**し、いきなり保持期間いっぱいまで段階検索 | `fullRetentionSearch: true`（明示パース時のみ） |

- **DB の保持**（`message_retention_days` 等）と **会話検索の届く範囲**は別。保持が 2 年でも、**第1段は最大半年相当**まで。
- 詳細なパース条件は `line-webhook` の `detectFullRetentionSearchRequest` / `extractMessageSearchKeyword` を参照。

### 8.1 データ取得（段階検索）

- 第一段階は `line_messages` のみ対象。**適用する検索上限日数**は、`resolveEffectiveMessageSearchDays` のあと、**通常モード**では **`MESSAGE_SEARCH_NORMAL_MAX_DAYS`（180 日）まで**に頭打ちした `effectiveDays` を使う（保持が 2 年でも、第1段は最大半年相当まで）。**明示の `会話検索フル`** のときは保持設定に従った日数からいきなり段階検索。
- **通常モードの第1段**（`buildMessageSearchPhase1WizardWindows`）は、当該 `effectiveDays` 分を **1 段で一括走査**。ヒット 0 件で、保持期間の方が第1段より長い場合は **第2段として同一リクエスト内で** `buildMessageSearchStageDayWindows`（フル相当）を続けて実行。それでも 0 件なら **第3段**として **資料ライブラリ検索**の `はい` / `いいえ` 確認へ進む（第1段の範囲が保持と同じなら第2段は省略して第3段へ）。
- **フルモード**（明示コマンド）の **内部の段階**（`buildMessageSearchStageDayWindows`）は `effectiveDays` に応じて **約 3 ヶ月 → 約 6 ヶ月 →（さらに長いとき）最終段**のように段を組み立て、**ヒットが出た段階で終了**（AI 要約もこの結果のみに対して実行）。
  1. **約 3 ヶ月**（90 日、`effectiveDays` より長い場合は上限に合わせる）
  2. **約 6 ヶ月**（180 日、同様にクリップ）
  3. **フルモードかつ**より長い `effectiveDays` が残るときのみ **最終段**（日付下限は `effectiveDays` に応じた `sinceIso`、無期限相当は日付下限なし。暴走防止のため **1 段階あたり最大 `MESSAGE_SEARCH_STAGE_HARD_MAX_ROWS`（200,000）件**までをバッチ読み取り）
- 旧仕様の「先頭 800 件だけ」という **件数上限は廃止**（期間と段階で制御）。
- **DB 事前絞り込み**: `search_line_messages_keyword_window`（マイグレーション `20260415140000_*`）で、期間・ルーム条件に加え **キーワードのトークン AND / 類義語 OR** を SQL の `ILIKE` で絞り込み、候補行のみを Edge に返す。RPC が未適用の環境では従来のバッチ取得にフォールバック。返却行は従来どおり **`messageMatchesKeyword` で再検証**（NFKC・コンパクト一致など）。
- スコープが `current_room` のときは `room_id` で絞り込み。
- 段階検索で**早い段階でヒットした**場合、まだ古い会話帯は走査していないことがあります。そのときは確認応答を経て、**未走査の古い帯だけ**を順に追加検索できます（`はい` / `いいえ`、および **「さらに検索」「続けて検索」** など短文の肯定。`続けて` 単独は肯定に含めない。30 分有効。テーブル: `message_search_expand_pending_confirmations`）。
- 会話検索（第1段・必要なら第2段）でヒット 0 件の場合のみ、**資料ライブラリ検索**へ進むか `はい` / `いいえ` で確認（30 分有効。テーブル: `message_search_library_pending_confirmations`）。**明示の `会話検索フル` で始めた場合**は第1・2段を挟まず、会話 0 件ならこの確認へ。
- 資料ライブラリ検索では `line_search_documents` から最大 **`SEARCH_MAX_DOCUMENT_ROWS`（300）** 件を取得し、**ファイル名に `[LINE]` を含む資料のみ**対象にする。
- 資料ライブラリ検索は会話保持期間（日数）で絞り込まず、**登録済み `[LINE]` 資料全体**を対象とする。
...
`message_retention_days` の影響は、第一段階の `line_messages` 検索に適用されます。  
資料ライブラリ検索（第二段階）は日数クリップを行わず、登録済み `[LINE]` 資料全体を対象にします。
...
- **会話検索（3段階）**: 第1段で過去180日（または保持が短い場合はその範囲）を一括検索 → 0件かつ保持がより長ければ第2段でフル相当を **同一応答内で** 続行 → 会話でも0件なら第3段で資料ライブラリの `はい`/`いいえ`。会話本文の取得は **Postgres RPC でキーワード事前絞り込み**（未適用時は従来バッチ）。
- **ドキュメント**: `line_messages` / 資料ライブラリの保存形と「検索は LLM 非使用」による **Groq 使用量の抑え**を `README.md` §7.1・§8.5、`ADMIN_UI_GUIDE.md` §5.8、`APP_ANALYSIS.md` に追記。
- **会話検索（通常は半年まで）**: 保持期間が長くても通常の会話検索は過去 180 日までに制限。**会話検索フル／全履歴／裏**などで保持期間いっぱい（例: 2 年分）を段階検索するフルモードを追加。
- **会話検索（古い帯・運用修正）**: 「続けて」単独での肯定扱いをやめ（誤反応防止）、追加帯の `next_ring_k` 保存失敗時は pending を閉じて案内。RLS ポリシー名を 63 文字以内の固定名に整理。
- **会話検索（古い帯の追加）**: 段階検索が早い段階でヒットした場合でも、未走査の古い帯を `はい` / `さらに検索` で順に追加検索できるようにした（`message_search_expand_pending_confirmations`）。
...
- **会話検索**: 800 件打ち切りを廃止し、**約3ヶ月→約6ヶ月→設定範囲**の段階検索＋期間ベース。段階あたり最大 200,000 件の安全上限のみ残す。

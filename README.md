...
5 分を過ぎても返信がない場合は破棄せず、件名末尾に `（仮）` を付けて自動登録します（`calendar-pending-cron` が毎分確認）。
...
- 判断が低信頼の場合は pending（確認待ち）に入り、`はい` / `いいえ` で確定・中止できます。返信がない場合は 5 分後に `（仮）` 付きで自動登録されます。
...
管理UIでは誤設定防止のため、`bot_reply_enabled` / `message_search_enabled` / `message_search_library_enabled` は表示せず固定制御しています。
...
- 管理UIからのルーム保存時は `bot_reply_enabled=true` / `message_search_enabled=false` / `message_search_library_enabled=false` を固定送信する。

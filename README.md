...
- **友だち 1:1**では `buildDirectUserRoomPolicy` により **会話検索・資料ライブラリ検索を無効化**。会話検索（全ルーム横断）の RPC / レガシ取得では **`room_id` が `U` 始まり（LINE の userId 由来の 1:1）の行を除外**し、グループ等の履歴のみヒットさせる。
- 保存対象メディア種別: `image`, `video`, `audio`, `file`（定数 `STORABLE_LINE_MEDIA_TYPES`）。取得後 `line-media` バケットへアップロードし `line_message_media` にメタデータ保存。
- トークから **保存メディアの署名付きダウンロード URL** を返す文言: `メディアURL`（省略時は**このトーク**の最新 1 件）、`メディアURL 2`〜`3`（当該ルームのみ・新しい順に最大 3 件）。**`メディアURL 全ルーム`** または **`メディアURL 3 全ルーム`** のように **`全ルーム` を付けると**、`line_message_media` に保存されたメディアを **トークルームの区別なく**（1:1・グループ等を含む）**新しい順**に最大 3 件返す。会話検索の「検索対象外ルーム」は **適用しない**。**友だち 1:1 でも**、メディア保存が有効で `can_media_access` があれば保存・URL 返却可。1:1 の**テキスト**は履歴 DB に載せず、**メディア行のみ**保存する。

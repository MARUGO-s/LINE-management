...
- 第一段階は `line_messages` のみ対象。**保持日数などで決まる検索上限日数（`effectiveDays`）**の範囲内で、**内部の段階**として次の順に試行する（**ヒットが出た段階で終了**。AI 要約もこの結果のみに対して実行）。
  1. **約 3 ヶ月**（90 日、ただし上限日数より長い場合は上限に合わせる）
  2. **約 6 ヶ月**（180 日、同様に上限でクリップ）
  3. **設定上の全期間**（`effectiveDays` が 0 のときは最終段で日付下限なし＝保存されている範囲。暴走防止のため **1 段階あたり最大 `MESSAGE_SEARCH_STAGE_HARD_MAX_ROWS`（200,000）件**までをバッチ読み取り）
- 旧仕様の「先頭 800 件だけ」という **件数上限は廃止**（期間と段階で制御）。
- スコープが `current_room` のときは `room_id` で絞り込み。
- 第一段階（上記の会話検索）でヒット 0 件の場合のみ、確認応答を経て**資料ライブラリ検索**へ進む（`はい` / `いいえ`、30 分有効）。

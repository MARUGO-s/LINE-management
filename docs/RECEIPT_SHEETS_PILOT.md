...
|--------|----------|-------------------|-------------------|-------------------|
| **操作** | B2〜B4 のチェックのみ | チェックで **取込／書出／双方向** を実行 | なし | 同期成功後に分析へ（同期内容による） |
| **使い方** | 任意 | なし | なし | なし |
...
スプレッドシートを開くと **「売上連携（パイロット）」** メニューが付きます（**PC ブラウザのみ**。スマホの Sheets アプリにはメニューは出ません）。

### スマホから同期する（「操作」タブ）

PC のカスタムメニューはスマホの Google スプレッドシートアプリには出ません。代わりに **「操作」** タブのチェックボックスで、PC メニューと同じ同期を実行します。
...
|----|--------|------|
| A | 操作 | 同期の種類（3行だけ） |
| B | 実行 | チェックボックス（**B2〜B4 のみ**） |
| C | 状態 | `実行中…` → 成功時 `完了`、失敗時はエラー文 |
...
**5行目以降は空** です。B5 以降にチェックが残っているのは **旧レイアウトの名残** です（下記「古い操作タブの直し方」）。

#### 使い方

1. **初回（PC）**: 最新の `Code.gs` を Apps Script に保存し、スプレッドシートを開く（またはメニュー **「操作シートを作成・更新（スマホ用）」** を1回実行）。  
   - **編集トリガー**（`handleOperationSheetEdit`）が未設定なら **自動でインストール** されます。  
   - シートを開いたとき（`onOpen`）に、旧形式の「操作」タブがあれば **3行レイアウトへ自動更新** します。
2. **スマホ**: **「操作」** タブ → **B列のチェックを1つだけ ON**（すぐ OFF に戻る）→ **C列** が `実行中…` → `完了`（失敗時はエラー文）。
3. **PC で同期したとき**: 成功時のポップアップは **「同期完了」** のみ（JSON や詳細ログは出しません）。

**B列のチェックは同時に1つだけ** ON にしてください。複数 ON にすると意図しない二重実行になることがあります。

#### 自動で行われること（操作タブにボタンは不要）

次の作業は **取込／書出／双方向のたびに GAS が自動実行** します。旧版にあった「操作」タブ上の個別行（営業日数列の追加・日次のマイナス赤・トリガー説明・実行ログ行など）は **削除済み** です。

| 処理 | タイミング |
|------|------------|
| 過去売上タブに **営業日数（F列）** を挿入（旧6列→7列） | 取込時 |
| 月間予算タブに **営業日数（I列）** を挿入し H列休業日から計算（旧9列→10列） | 取込時 |
| 日次売上の **マイナスを赤文字** | 書出時 |
| 同期の記録 | **「同期ログ」** タブへ1行追加（「操作」タブではない） |

#### 古い「操作」タブの直し方

以前のパイロットでは「操作」タブに **6行以上**（列追加・マイナス赤・トリガー・使い方・実行ログ・**説明** 列など）がありました。今は **3行×3列** のみです。

| 症状 | 対処 |
|------|------|
| 4行目以降に **説明** や **トリガー** の行がある | 最新 `Code.gs` を保存 → シートを **閉じて開き直す**（`onOpen` で自動作り直し） |
| ラベルは3行だけだが **B5 以降に空のチェックが残る** | 同上。`sheet.clear()` ではチェックボックスのデータ検証が消えないため、GAS が **5行目以降の検証を明示削除** します |
| 自動更新されない | メニュー **「操作シートを作成・更新（スマホ用）」** を1回実行 |

Apps Script エディタで `clearOperationSheetExtras_` という関数があるか確認すると、**新しい `Code.gs` が反映されているか** の目安になります。
...
| **操作シートを作成・更新** | — | — | スマホ用「操作」タブの再作成・余分なチェック削除・編集トリガー設定 |
...
- **「操作」タブ**: `setupOperationSheet_` が B2:B4 にだけチェックボックス検証を付与。`onOpen` → `ensureOperationSheetCurrent_` が旧レイアウト検出時に作り直し、ラベルは正しくても B5 以降に検証が残る場合は `clearOperationSheetExtras_` で削除。
- **編集トリガー**: インストール型 `onEdit` → `handleOperationSheetEdit`（B列・行2〜4・`true` のときのみ）。PC メニュー同期は従来どおり `syncBoth` / `syncPull` / `syncPush`。
...
C 列（総売上円）を **空** にして取込すると、その月の手入力を **削除** します（組数・客数・営業日数も含む）。

F 列（営業日数）を入れると、レシート返信の途中期間前年比は **暦日数比ではなく営業日比** で按分します（例: 今年14営業日 ÷ 昨年手入力20営業日）。
...
| A | B | C | D | E | F | G | H | I | J |
|---|---|---|---|---|---|---|---|---|---|
| 対象月 | 店舗名 | 店舗キー | 月間予算円 | 平日比率 | 休日前比率 | 休日比率 | 休業日 | **営業日数** | 有効 |

**I列（営業日数）** は **H列の休業日** から自動計算（暦日数 − 休業日数）。取込・双方向同期の前後で GAS が I 列を更新します。取込時、同じ月の **過去売上** の営業日数が空欄ならこの値で補完します。

旧9列（I=有効）も取込可能。取込時に GAS が **I 列（営業日数）を自動挿入** します。
...
| A | B | C | D | E | F | G |
|---|---|---|---|---|---|---|
| 対象月 | 店舗キー | 総売上円 | 会計組数 | 客数 | **営業日数** | 有効 |

2行目の例（**前年5月** を前年比用に登録）:

```
2025-05	bistrocavacava	1500000	50	120	20	TRUE
```

- **B列** 店舗キー: `bistrocavacava`  
- **C列** 総売上円: 空にして取込するとその月の手入力を **削除**  
- **D・E列** 会計組数・客数: 空欄可  
- **F列** 営業日数: その月の営業日数（1以上）。レシート返信の途中期間前年比は **今年の営業日数 ÷ この値** で按分（未入力時は暦日数比 `20/31` など）  
- **G列** 有効: `TRUE`  
- 比較したい月ごとに行を追加  
- **旧6列**（F=有効）・**旧4列**（D=有効）も取込可能（営業日数・組数・客数は未登録扱い）
...
4. スプレッドシートを再読み込み → **「操作」タブ** が無ければ自動作成 → メニュー **売上連携（パイロット）** → **双方向同期**（スマホ運用なら **操作シートを作成・更新** も一度実行推奨）
...
**Q. 操作タブの下にチェックボックスだけ残る**  
A. 旧版の検証が残っています。最新 `Code.gs` を保存し、シートを開き直すか **操作シートを作成・更新** を実行してください。

**Q. スマホで同期したあと何が表示される？**  
A. C列は **完了**（PC メニューからの成功時はポップアップ **同期完了**）。詳細は **同期ログ** タブを参照。

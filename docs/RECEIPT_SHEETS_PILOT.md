...
C 列（総売上円）を **空** にして取込すると、その月の手入力を **削除** します（組数・客数も含む）。
...
| A | B | C | D | E | F |
|---|---|---|---|---|---|
| 対象月 | 店舗キー | 総売上円 | 会計組数 | 客数 | 有効 |

2行目の例（**前年5月** を前年比用に登録）:

```
2025-05	bistrocavacava	1500000	120	450	TRUE
```

- **B列** 店舗キー: `bistrocavacava`  
- **C列** 総売上円: 空にして取込するとその月の手入力を **削除**  
- **D・E列** 会計組数・客数: 空欄可（未入力時はレシート集計を前年比に使用）  
- **F列** 有効: `TRUE`  
- 比較したい月ごとに行を追加  
- 旧4列シート（有効がD列）も取込可能（組数・客数は未登録扱い）

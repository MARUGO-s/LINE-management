...
| 客単価 | 客数 > 0 のとき |
| 1日平均売上 | 営業日数ベース |
| 営業日数 | 期間内でレシートがある日数（`receipt_date` の distinct 数。通常はレシート件数と一致） |

※ 単独行の「組数平均」は **表示しない**（2026-05 以降）。

### 4.1.1 【前年同月比】

前年の **同一日付範囲**（例: 2026/5/1〜5/15 → 2025/5/1〜5/15）と比較します。対象の前年月に **手入力の過去売上**（`line_sales_manual_month_gross`）がある場合:

- **売上**: 手入力（日次レシート返信の途中期間は按分）。**過去売上に営業日数（F列）があるとき**は `今年の営業日数 ÷ 手入力の営業日数`。未入力時は暦日数比（例: 5/20 時点なら `20÷31`）。
- **組数・客数**: 手入力に値がある列だけ使用。**総売上のみ**登録のときは **「—」**。途中期間の按分は **営業日数列** 優先（`今年営業日 ÷ 手入力営業日`）。
- **営業日数**: 引き続き昨年レシート日付のみ（手入力なし）。

| 行 | 内容 |
|----|------|
| 昨年差異日 | 前年の集計期間 |
| 売上 / 組数 / 客数 / **営業日数** | 前年比（%）と差分 |
